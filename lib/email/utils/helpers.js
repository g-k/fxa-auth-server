...
function logFlowEventFromMessage(log, message, type, emailDomain) {
  ...
  if (flowId && flowBeginTime && (elapsedTime > 0) && type && templateName) {
    const eventName = `email.${templateName}.${type}.${emailDomain}`
    ...
    // If a locale exits and log it
    var locale = getHeaderValue('Content-Language', message)
    if (locale) {
      flowEventInfo.locale = locale
    }
...
function getAnonymizedEmailDomain(email) {
  ...
  var popularEmailDomain = 'other'
  if (emailDomain && emailDomains[emailDomain]) {
    popularEmailDomain = emailDomains[emailDomain]
  }

  return popularEmailDomain
}
